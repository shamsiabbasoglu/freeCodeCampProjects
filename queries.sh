...
echo "$($PSQL "SELECT name FROM games AS g FULL JOIN teams AS t ON g.winner_id=t.team_id 
...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g
LEFT JOIN teams t ON t.team_id = g.winner_id
ORDER BY t.name")"
